...
class Utils:
    ...
    def extract_bb_points(self, points, width, height):
        if len(points.split(';')) == 4: # 라벨링된 point가 4개인 경우 = polygon 라벨링 (없음)
            ...
            top_two = sorted_by_y[0:2] #  밑에 있는 두 점
            topLeft, topRight = self.sort_ndarray_by_col(top_two, 0)
            bottom_two = sorted_by_y[2:4] 
        ...
        b1 = (bottomLeft[0], bottomLeft[1]+y_offset) # bottomLeft
        b2 = (bottomRight[0], bottomRight[1]+y_offset) # bottomRight
        b3 = (b2[0], b2[1]+height) # TopRight
        b4 = (b1[0], b1[1]+height) # TopLeft
